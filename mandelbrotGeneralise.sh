...
do
    s=$((s+$i))
    java -jar build/libs/projets-des-fractales-sae.jar -f "mandelbrotGeneralise" -h ${h} -n ${n} -o "$DOSSIER/mandelbrotGeneralise-$((nb+=1))" -p ${p} -s ${s} -w ${w} -x ${x} -y ${y}
done
